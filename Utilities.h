...
bool eeprom_product_valid() {
	...
	#elif PLATFORM == PLATFORM_NRF52
	if (rval == PRODUCT_RAK4631 || rval == PRODUCT_HELTEC_T114 || rval == PRODUCT_OPENCOM_XL || rval == PRODUCT_TECHO || rval == PRODUCT_FAKETEC || rval == PRODUCT_HMBRW) {
	#else
	if (false) {
	#endif
		return true;
...
bool eeprom_model_valid() {
    ...
    if (model == MODEL_21) {
	#elif BOARD_MODEL == BOARD_FAKETEC_V5
	if (model == MODEL_F4 || model == MODEL_F5) {
	#elif BOARD_MODEL == BOARD_HUZZAH32
	if (model == MODEL_FF) {
	#elif BOARD_MODEL == BOARD_E22_ESP32
	if (model == MODEL_FF || model == MODEL_FE) {
